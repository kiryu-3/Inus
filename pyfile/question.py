...
class question:
    ...
    def questionmaker(self, user_info):
        df = pd.read_csv(PATH)

        for idx, category in enumerate(df["カテゴリ"].unique()):
            question_dict = {}
            ...
            file_path = "result/json/question" + str(idx+1) + ".json"

            self.save(question_dict, file_path, f"question{str(idx+1)}")
        

    def questionreviser(self, category_idx, skills, user_info):
        ...
        file_path = "result/json/question" + str(category_idx) + ".json"

        self.save(question_dict, file_path, f"question{str(category_idx)}")